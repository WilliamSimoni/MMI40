...
app.post('/get', [
    ...
    try {
        ...
        if (store === true) {
            ...
            for (item of queryResult.result.results) {
                ...
                if (item.series) {
                    ...
                    if (granularityIsNumeric) {
                        timeToStart = time.nearestMoment(time.round(start,rounder.roundGran1(timePeriodInSecond)), granularity.number, granularity.key, start);
                    } else {
                        timeToStart = time.nearestMoment(time.round(start,rounder.roundGran2(granularityInSecond)), granularity.number, granularity.key, start);
                    }
                }
            }
        }
        ...
        response.status(200).json({ status: 200, initialData, timeToStart });
